...
if [ "$BUILD_PICO" == "ON" ] || [ "$BUILD_PICO" == "TRUE" ] || [ "$BUILD_PICO" == "1" ]; then
    #build zenoh-pico
    bash $SCRIPT_DIR/install_local.sh $SCRIPT_DIR/../zenoh-pico $absolute_install_location $BUILD_TYPE -DZ_FEATURE_UNSTABLE_API=$USE_UNSTABLE_PICO -DZ_FEATURE_BATCHING=1 -DZ_FEATURE_ADVANCED_SUBSCRIPTION=$USE_UNSTABLE_PICO -DZ_FEATURE_ADVANCED_PUBLICATION=$USE_UNSTABLE_PICO -DZ_FEATURE_MATCHING=1
fi
